...
import com.idvkm.bgidc.databinding.FragmentEditSessionBinding
...
class EditSessionFragment : Fragment() {
    ...
    private fun loadSpeakers() {
        FirebaseCalls().getSpeakers(onSuccess = { speakers ->
            ...
            val adapter = ArrayAdapter(requireContext(), R.layout.simple_dropdown_item_1line, names)
            binding.editMultiSpeakerSelect.setAdapter(adapter)
            binding.editMultiSpeakerSelect.setTokenizer(MultiAutoCompleteTextView.CommaTokenizer())
